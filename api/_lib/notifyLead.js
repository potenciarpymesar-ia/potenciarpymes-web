...
async function notifyLead(lead) {
  ...
  if (!resendKey) {
    console.error('notifyLead: falta RESEND_API_KEY (y no hay N8N_WEBHOOK_URL) — lead solo queda en logs', lead);
    return;
  }
  ...
  try {
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: notifyTo,
        subject: `Nuevo lead auditoría web — ${lead.nombre} (${lead.auditId})`,
        html,
      }),
    });
  } catch (err) {
    console.error('notifyLead: fallo el envío por Resend', err, lead);
  }
}
